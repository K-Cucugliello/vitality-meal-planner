...
export default function MealForm({ onSubmit, onCancel }) {
    ...
    const handleAiImport = async () => {
        ...
        try {
            ...
            setFormData(prev => ({
                ...
                description: instructions || prev.description,
                ...
                ingredients: data.extendedIngredients?.map(ing => ({
                    ...
                    amount: `${ing.measures.metric.amount} ${ing.measures.metric.unitShort}`
                })) || prev.ingredients
            }));
            alert("Imported!");
        } catch (error) {
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            ...
            <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                <Label className="text-emerald-800 font-bold flex items-center gap-2 mb-2">
                    <Sparkles className="w-4 h-4" /> AI Recipe Import
                </Label>
                <div className="flex gap-2">
                    <Input placeholder="Paste URL..." value={recipeUrl} onChange={(e) => setRecipeUrl(e.target.value)} className="bg-white" />
                    <Button type="button" onClick={handleAiImport} disabled={isAiLoading} className="bg-emerald-600">
    ...
    <select 
        ...
        className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm ring-offset-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
    >
        ...
        <option value="snack">🍎 Snack</option>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {['calories', 'protein', 'carbs', 'fat'].map((f) => (
                    <div key={f} className="space-y-1">
                        ...
                        <Input type="number" value={formData[f]} onChange={(e) => setFormData({...formData, [f]: e.target.value})} />
                    </div>
                ))}
            </div>
            ...
            <div className="space-y-3">
                <div className="flex justify-between">
                    ...
                    <Button type="button" variant="ghost" size="sm" onClick={() => setFormData({...formData, ingredients: [...formData.ingredients, {name: '', amount: ''}]})}>
    ...
    <div key={idx} className="flex gap-2 items-center">
        ...
        >
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
                ...
                <Button onClick={() => onSubmit(formData)} className="bg-emerald-600 text-white">Create Meal</Button>
            </div>
        </motion.div>
    );
}